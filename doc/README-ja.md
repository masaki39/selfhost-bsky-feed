...
2. `npm run worker:publish` で手動デプロイ、またはワークフロー内で feed.json 生成後に実行するステップを追加。
3. 付属のワークフロー `.github/workflows/02_publish-worker.yml` は GitHub コンテキストから `GITHUB_OWNER`/`GITHUB_REPO` を注入し、Worker 名を `${{ github.event.repository.name }}-worker` に設定します。手動トリガーまたは `01_update-feed.yml` 成功時に自動実行します。

GitHub Pages で `data/feed.json` をホストする例（任意、Worker との組み合わせ推奨）:
1. Pages を有効化し、「Build and deployment」を “GitHub Actions” に設定。  
2. feed 生成後に `data/feed.json` を Pages 用アーティファクトとしてアップロード（例: `actions/upload-pages-artifact@v3` と `actions/deploy-pages@v4` を追加）。  
3. Worker は GitHub Pages の URL をデフォルトで取得します（`https://<owner>.github.io/<repo>/data/feed.json` を構築）。
